...
void solve() {
    ...
    vi subset(n);
    ll total = 0;
    auto dfs = [&](auto& dfs, int i = 0, int s = 0) -> int {
        if(i == n) return s == v % k;
        auto& res = dp[i][s];
        if(res != -1) return res;
        if(dfs(dfs, i + 1, s)) return res = true;
        if(dfs(dfs, i + 1, (s + a[i]) % k)) {
            total += a[i];
            subset[i] = true;
            return res = true;
        }
    ...
    if(big == -1 || small == -1) {
        for(int i = 0; i < n; i++) {
            if(i == small || i == big || a[i] == 0) continue;
            ll need = (a[i] + k - 1) / k;
            int j = big == -1 ? small : big;
            a[j] += a[i];
            a[i] = 0;
            cout << need << ' ' << i + 1 << ' ' << j + 1 << '\n';
        }
        if(big == -1) big = small == 0 ? 1 : 0;
        else small = big == 0 ? 1 : 0;
    }

    for(int i = 0; i < n; i++) {
        if(i == big || i == small || a[i] == 0) continue;
        int j = subset[i] ? big : small;
        ll time = (a[i] + k - 1) / k;
        cout << time << ' ' << i + 1 << ' ' << j + 1 << '\n';
        a[j] += a[i];
        a[i] = 0;
    ...
    if(total > v) {
        ...
        a[small] += need * k;
        a[big] -= need * k;
        cout << need << ' ' << big + 1 << ' ' << small + 1 << '\n';
    } else if(total < v) {
        ll need = (v - total) / k;
        a[big] += need * k;
        a[small] -= need * k;
        cout << need << ' ' << small + 1 << ' ' << big + 1 << '\n';
    }
}
