...
const int MK = 22;
int T[MX], root[MX * MK * 4], ptr, n, q, m;
pii child[MX * MK * 4];  
void update(int prev, int curr, int id, int left = 0, int right = m)
{   
    root[curr] = root[prev];    
    ...
    {   
        child[curr].ff = ++ptr; 
        update(child[prev].ff, child[curr].ff, id, left, middle);   
    }   
    else    
    {   
        child[curr].ss = ++ptr; 
        update(child[prev].ss, child[curr].ss, id, middle + 1, right);  
    }   
    root[curr] = root[child[curr].ff] + root[child[curr].ss];
}   
    
void print(int curr, int left = 0, int right = m)
{   
    if(left == right)   
    {   
        cout << left << " " << root[curr] << " | ";  
    ...
    int middle = midPoint;  
    print(child[curr].ff, left, middle);    
    print(child[curr].ss, middle + 1, right);   
}

    
int queries(int curr, int k, int res = 0, int left = 0, int right = m)
{   
    if(left == right) return res + root[curr];  
    int middle = midPoint;
    int rightSum = root[child[curr].ss];    
    if(k <= middle) return queries(child[curr].ff, k, res + rightSum, left, middle); 
    return queries(child[curr].ss, k, res, middle + 1, right);
}
...
{   
    ...
    {   
        int middle = midPoint;  
        int val = queries(T[middle], x);    
        if(val >= k) res = middle, right = middle - 1;    
        else left = middle + 1; 
    }   
    return res; 
}
...
void solve()
{
    ...
    m = *max_element(all(arr));
    int pre = 0;    
    for(int i = 0; i < n; i++)
    {   
        T[i] = ++ptr;   
        update(pre, T[i], arr[i]);
        pre = T[i];
    }
    while(q--)  
    {   
        int x = answer();   
        cout << (x == -1 ? x : arr[x]) << endl;
    }
