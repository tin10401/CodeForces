...
void solve() {
    int n, m; cin >> n >> m;
    vvi grid(n, vi(m));
    int min_x = inf, min_y = inf, total = 0;
    for(int i = 0; i < n; i++) {
        for(int j = 0; j < m; j++) {
            char x; cin >> x;
            if(x == 'X') {
    ...
    TWO_DIMENSIONAL_RANGE_QUERY g(grid);
    auto f = [&](int a, int b) -> bool {
        int X = min_x, Y = min_y, running_sum = 0;
        while(true) {
            if(X + a - 1 > n || Y + b - 1 > m) {
                return false;
            }
            if(g.get(X, Y, X + a - 1, Y + b - 1) != a * b) {
                return false;
            }
            if(g.get(X, Y, n, m) == a * b) return running_sum + a * b == total;
            bool go_right = Y + b - 1 < m && grid[X - 1][Y + b - 1];
            bool go_down = X + a - 1 < n && grid[X + a - 1][Y - 1];
            if(!go_right && !go_down) return false;
            if(go_right && go_down) return false;
            if(go_down) X++, running_sum += b;
            else Y++, running_sum += a;
        }
        return false;
    };
    int res = inf;
