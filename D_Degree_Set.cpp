...
void solve() {
    ...
    while(a[n - 1] > l) {
        do {
            debug(a[n - 1]);
            // by moving inserting from l + 2 to a[n - 1] + 1,
            // we'll have a[l, next(f[l])] having a[n - 1] degree
            // in addition to that we'll have a[n - 2] to a[n - 1] node have degree of next[a[l]]
            for(int i = l + 2; i <= a[n - 1] + 1; i++) {
                ans.pb({l + 1, i});
                degree[l + 1]++;
                degree[i]++;
            }
