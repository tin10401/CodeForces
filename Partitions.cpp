...
class SGT { 
    ...
    int queries(iterator, int start, int end) { 
        ...
        int middle = midPoint;  
        return (queries(lp, start, end) + queries(rp, start, end)) % MOD;
    }
};
...
void solve() {  
    ...
    int ans = 0;
    for(int i = n - 1; i >= 0; i--) {   
        int leftMost = lb(begin(prefix) + i + 1, end(prefix), prefix[i] + l) - begin(prefix);   
        int rightMost = ub(begin(prefix) + i + 1, end(prefix), prefix[i] + r) - begin(prefix);  
        if(leftMost == n + 1) continue; 
        rightMost--;
        ans = root.queries(leftMost, rightMost);
        root.update(i, ans);
    }
    cout << ans << endl;
}
