...
void solve() {
    int n, m; cin >> n >> m;
    vvi a(n, vi(m)), b(n, vi(m)), c(n, vi(m)); cin >> a >> b >> c;
    const int N = n * m;
    int src = N * 2, sink = N * 2 + 1; 
    dinic<ll> graph(N * 2 + 2);
    ll total = 0;
    ...
    for(int i = 0; i < n; i++) {
        for(int j = 0; j < m; j++) {
            total += a[i][j];
            int x = get_hash(i, j);
            graph.add_directional_edge(x, x + N, a[i][j]);
            if(i % 2 == j % 2) {
                graph.add_directional_edge(src, x, b[i][j]);
                graph.add_directional_edge(x + N, sink, c[i][j]);
            }
            else {
                graph.add_directional_edge(src, x, c[i][j]);
                graph.add_directional_edge(x + N, sink, b[i][j]);
                for(int k = 0; k < 4; k++) {
                    int r = i + dirs[k][0], c = j + dirs[k][1];
                    if(r >= 0 && r < n && c >= 0 && c < m) {
                        int y = get_hash(r, c);
                        graph.add_directional_edge(x, y, INF);
                        graph.add_directional_edge(x + N, y + N, INF);
                    }
                }
            }
        }
    }
    cout << total - graph.flow(src, sink) << endl;
}
