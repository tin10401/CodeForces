...
struct EulerianPath {
    ...
    void add_edge(int u, int v, int id, int good) {
        graph[u].pb({v, id, good});
        edges++;
        if(directed) {
            outdeg[u]++;
            indeg[v]++;
        } else {
            graph[v].pb({u, id, good});
            deg[u]++;
            deg[v]++;
        }
    }
    ...
    void dfs(int u) {
        while(!graph[u].empty()) {
            auto [v, id, good] = graph[u].back();
            graph[u].pop_back();
            while((int)used.size() <= id) used.pb(0);
            if(used[id]) continue;
            used[id] = true;
            dfs(v);
            if(good) ans_edges.pb({u, v});
        }
    }
};
...
void solve() {
    ...
    fill(all(degree), 0);
    for(auto& [u, v] : graph.ans_edges) {
        degree[u]++;
        degree[v]--;
    }
    int res = count(all(degree), 0);
    cout << res << '\n';
    for(auto& [u, v] : graph.ans_edges) {
        cout << u + 1 << ' ' << v + 1 << '\n';
    }
