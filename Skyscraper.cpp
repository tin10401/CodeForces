...
void solve() {
    ...
    auto dfs = [&](auto& dfs, ll i = 0, ll j = 0, ll k = 0, ll s = 0) -> int {
        ...
        k += edge * (a[i + 1] - a[i]);
        if(k <= l) {
            now += (j - 1) * dfs(dfs, i + 1, j - 1, k, s); // j - 1 choices to close the component
            now += (j + 1 - s) * dfs(dfs, i + 1, j + 1, k, s); // j + 1 position to insert this in
            now += edge * dfs(dfs, i + 1, j, k, s); // edge choices to keep the same component
            now += (2 - s) * dfs(dfs, i + 1, j, k, s + 1); // ?comp1_comp2_comp3_comp4? 2 choices each left_most and right_most
            now += (2 - s) * dfs(dfs, i + 1, j + 1, k, s + 1);
        }
        res = int(now);
        return res;
    };
