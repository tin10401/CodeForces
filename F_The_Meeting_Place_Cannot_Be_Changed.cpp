...
void solve() {
    int n, m; cin >> n >> m;
    vt<unordered_set<int>> out(n), in(n);
    while(m--) {
        int u, v; cin >> u >> v;
        u--, v--;
        out[u].insert(v);
    ...
    }
    {
        ...
        while(!q.empty()) {
            ...
            for(auto& nei : in[node]) {
                out[nei].erase(node);
                if(out[nei].empty()) {
                    q.push(nei);
                }
            }
            unordered_set<int>().swap(out[node]);
            unordered_set<int>().swap(in[node]);
        }
    }
    {
        stack<int> q;
        ...
        while(!q.empty()) {
            ...
            if(out[node].count(node)) continue;
            int next = *out[node].begin();
            in[next].erase(node);
            in[next].insert(all(in[node]));
            for(auto& p : in[node]) {
                out[p].erase(node);
                out[p].insert(next);
