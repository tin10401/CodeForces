...
void solve() {
    ...
    dinic<ll> graph(N + 2);
    for(int i = 0; i < n; i++) {
        graph.add_directional_edge(src, i, a[i]);
        graph.add_directional_edge(i + n, sink, b[i]);
        graph.add_directional_edge(i, i + n, inf);
    }
    vvi cost(n, vi(n));
    while(m--) {
        int u, v; cin >> u >> v;
        u--, v--;
        graph.add_directional_edge(u, v + n, inf);
        graph.add_directional_edge(v, u + n, inf);
    }
    if(graph.flow(src, sink) != sum(a)) {
        cout << "NO" << endl;
        return;
    }
    cout << "YES" << endl;
    for(auto &it : graph.assign_flow(n)) output_vector(it);
