...
template<typename T = int, typename F = ll>
F nC2_vector(vt<T>& a) { // compute sum(a[i] * a[j]) over all pair
    ll s = 0, S = 0;
    for (ll x : a) {
        s += x;
        S += x * x;
...
struct rerooting {
    ...
    void dfs1(int node, int par) {
        ...
        for(auto& nei : graph[node]) {
            if(nei == par) continue;
            dfs1(nei, node);
            if(max_height[nei] + 1 > max_height[node]) {
                max_height[node] = max_height[nei] + 1;
                cnt[node] = cnt[nei];
            } else if(max_height[nei] + 1 == max_height[node]) {
                cnt[node] += cnt[nei];
            }
        }
    }

    void dfs2(int node, int par, ll up_height, ll up_cnt) {
        vpll now;
        if(node > 0 || graph[node].size() == 1) {
            now.pb({up_height, up_cnt});
        }
        for(auto& nei : graph[node]) {
            if(nei == par) continue;
            now.pb({max_height[nei] + 1, cnt[nei]});
        }
        srtR(now);
        if(now.size() >= 3) {
            auto a = now[0], b = now[1], c = now[2];
            vll t;
            ll tie = 0;
            for(auto& it : now) {
                if(it.ff == c.ff) {
            ...
            ll nc2 = nC2_vector(t);
            ll max_height_now = a.ff * (b.ff + c.ff);
            ll cnt_now = 0;
            if(a.ff == b.ff && b.ff == c.ff) {
                cnt_now = nc2;
            } else if(a.ff == b.ff) {
                cnt_now = (a.ss + b.ss) * tie;
            } else if(b.ff == c.ff) {
                cnt_now = nc2;
            } else {
                cnt_now = b.ss * tie;
            }
            if(max_height_now > ans) {
                ans = max_height_now;
                ans_cnt = cnt_now;
            } else if(max_height_now == ans) {
                ans_cnt += cnt_now;
            }
        }
        int len1 = 0, len2 = 0, cnt1 = 0, cnt2 = 0;
        for(auto& [x, y] : now) {
            x++;
            if(x > len1) {
                len2 = len1;
                ...
                len1 = x;
                cnt1 = y;
            } else if(x == len1) cnt1 += y;
            else if(x > len2) {
                len2 = x;
                cnt2 = y;
            } else if(x == len2) cnt2 += y;
        }
        for(auto& nei : graph[node]) {
            if(nei == par) continue;
            if(max_height[nei] + 2 == len1) {
                if(cnt1 == cnt[nei]) {
                    dfs2(nei, node, len2, cnt2);
                } else {
                    dfs2(nei, node, len1, cnt1 - cnt[nei]);
                }
            } else {
                dfs2(nei, node, len1, cnt1);
            }
        }
    }
};
...
void solve() {
    ...
    rerooting<int> g(graph);
    cout << g.ans << ' ' << g.ans_cnt << '\n';
}
