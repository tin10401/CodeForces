...
void solve()
{
    ...
    FW f(m);    
    for(auto& it : arr) f.update(mp[it], 1);
    for(int i = 0; i < q; i++)  
    {   
        int op = list[i][0];    
        if(op == 0) 
        {   
            int x = list[i][1], y = list[i][2];
            f.update(mp[arr[x]], -1);
            f.update(mp[y], 1);
            arr[x] = y;
        }   
        ...
        {   
            int a = list[i][1], b = list[i][2], c = list[i][3], d = list[i][4]; 
            int left = mp[max(a, c)], right = mp[min(b, d)];    
            int res = f.get(right) - f.get(left - 1);   
            cout << (res >= k ? "Propose" : "Do not propose") << endl;
        }   
    }
}
