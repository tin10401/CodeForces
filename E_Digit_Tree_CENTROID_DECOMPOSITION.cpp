...
struct CD { // centroid_decomposition
    ...
    int mx;
    int get_down_value(int d, int depth) {
        assert(d < m);
        return (((ll)m - d) * inv[depth] ) % m;
    }
    int get(const map<int, int>& mp, int v) {
        auto it = mp.find(v);
        return it == end(mp) ? 0 : it->ss;
    }
    void modify(int node, int par, ll u, ll d, int depth = 1) {
        assert(u < m && d < m);
        up[u]++;
        down[get_down_value(d, depth)]++;
        for(auto& [nei, w] : graph[node]) {
            if(vis[nei] || nei == par) continue;
            modify(nei, node, (w * p[depth] + u) % m, (d * 10 + w) % m, depth + 1);
        }
    }
    void cal(int node, int par, ll u, ll d, int depth = 1) {
        assert(u < m && d < m);
        ans += get(up, get_down_value(d, depth)) + get(down, u);
        for(auto& [nei, w] : graph[node]) {
            if(vis[nei] || nei == par) continue;
            cal(nei, node, (w * p[depth] + u) % m, (d * 10 + w) % m, depth + 1);
        }
    }
