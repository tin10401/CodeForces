...
void solve() {  
    int n; cin >> n;    
    vi arr(n); cin >> arr;  
    vi prefix_sum(n + 1), prefix_xor(n + 1);    
    for(int i = 1; i <= n; i++) {   
        prefix_sum[i] = prefix_sum[i - 1] + arr[i - 1]; 
        prefix_xor[i] = prefix_xor[i - 1] ^ arr[i - 1];
    }
    int l, r; cin >> l >> r;    
    auto get = [&](int left, int right) -> int {    
        return prefix_sum[right] - prefix_sum[left - 1] - (prefix_xor[right] ^ prefix_xor[left - 1]);
    };
    int start = l, end = r, ans = r - l + 1;
    for(int i = l; i <= r; i++) {   
        int left = i, right = r, nxt = -1;  
        while(left <= right) {  
            int middle = midPoint;  
            if(get(i, middle) == get(l, r)) nxt = middle, right = middle - 1;  
            else left = middle + 1;
        }
        if(nxt != -1 && (nxt - i + 1) < ans) {   
            ans = nxt - i + 1;
            start = i;
            end = nxt;
        }
