...
void solve() {
    // find maximum length of palindromic subsequence of all path in the graph
    // transfer it to dp[i][j] = max({dp[i + 1][j], dp[i][j - 1], (s[i] == s[j] ? dp[i + 1][j - 1] + 2 : 0)}) type of problem
    int n; cin >> n;
    string s; cin >> s;
    vvi graph(n);
    for(int i = 0; i < n - 1; i++) {
    ...
    for(int d = 1; d <= n; d++) {
        ...
        for(auto& [u, v] : dist[d]) {
            ...
            dp[u][v] = dp[v][u] = max({dp[u1][v], dp[u][v1], s[u] == s[v] ? dp[u1][v1] + 2 : 0});
        }
    }
    int res = 0;
    for(int i = 0; i < n; i++) 
