...
int a[MX], parent[MX], t[MX][2], nxt = 1, dp[MX];    
vi children[MX];
...
int dfs(int node, int last, int cnt)    
{   
    if(a[node] == 1)    
    {   
        if(last > 0)    
        {   
            dp[node] = 1;   
            if(parent[last] != -1 && a[parent[last]] == 1)  
            {   
                dp[node] += dp[parent[last]];   
            }   
        }   
        last = node;    
    }   
