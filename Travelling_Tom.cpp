...
void solve() {
    ...
    auto dfs = [&](auto& dfs, vb& vis, int mask, int node = 1, int par = -1) -> int { 
        vis[node] = true;
        int s = 1;
        for(auto& [nei, cost] : graph[node]) {  
            if(vis[nei] || ((cost | mask) != mask)) continue;  
            s += dfs(dfs, vis, mask, nei, node);
        }
        return s;
    };
    int mask = (1LL << k) - 1, res = 0;
    vb v(n + 1);  
    if(dfs(dfs, v, mask) != n) {  
        cout << -1 << endl; 
        return;
