...
void solve() {
    ...
    const int N = n + 5;
    MCMF graph(N);
    int src = n, sink = n + 1, A = n + 2, B = n + 3, C = n + 4;
    for(int i = 0; i < n; i++) {
        int t; cin >> t;
        graph.add_edge(i, A, 1, -t);
    }
    for(int i = 0; i < n; i++) {
        int t; cin >> t;
        graph.add_edge(i, B, 1, -t);
    }
    for(int i = 0; i < n; i++) {
        int t; cin >> t;
        graph.add_edge(i, C, 1, -t);
    }
    for(int i = 0; i < n; i++) {
        graph.add_edge(src, i, 1, 0);
    }
    graph.add_edge(A, sink, size[0], 0);
    graph.add_edge(B, sink, size[1], 0);
    graph.add_edge(C, sink, size[2], 0);
    cout << -graph.min_cost_flow(src, sink, sum(size)).ss << endl;
}
