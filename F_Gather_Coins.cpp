...
void solve() {
    ...
    for(int i = 0; i < n; i++) {   
        auto [_, x] = arr[i];
        x = mp[x];  
        auto [coin, j] = root.get(x);   
        dp[i] = j;
        root.update(x, {coin + 1, i});
    }
    vi path = {n - 1};  
    while(path.back()) {    
        path.pb(dp[path.back()]);
    }
    rev(path);
    n = path.size();    
    cout << n - 2 << endl;
    for(int i = 1; i < n; i++) {   
        int x = arr[path[i]].ff - arr[path[i - 1]].ff;
        int y = arr[path[i]].ss - arr[path[i - 1]].ss;  
        cout << string(x, 'D') + string(y, 'R');
    }
    cout << endl;
}
