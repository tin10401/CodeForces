...
void solve() {
    int n; cin >> n;
    vi a(n); cin >> a;
//    vvmint dp(n + 1, vmint(n + 1));
//    dp[0][0] = 1;
//    for(auto& x : a) {
//        auto ndp(dp);
//        for(int mx1 = 0; mx1 <= x; mx1++) {
//            for(int mx2 = 0; mx2 <= mx1; mx2++) {
//                ndp[x][mx2] += dp[mx1][mx2];
//            }
//        }
//        for(int mx1 = x + 1; mx1 <= n; mx1++) {
//            for(int mx2 = 0; mx2 <= x; mx2++) {
//                ndp[mx1][x] += dp[mx1][mx2];
//            }
//        }
//        swap(dp, ndp);
//    }
...
//    }
//    cout << res << '\n';
    BIT2D<mint> fw(n + 1, n + 1);
    fw.update(0, 0, 1);
    for(auto& x : a) {
        var(3) now;
        for(int mx2 = 0; mx2 <= x; mx2++) {
            // dp[x][mx2];
            auto v = fw.query(x, mx2) - fw.query(x, mx2 - 1);
            now.pb({x, mx2, (int)v});
        }
        for(int mx1 = x + 1; mx1 <= n; mx1++) {
            // dp[mx1][x]
            auto v = fw.query(mx1, x) - fw.query(mx1 - 1, x);
            now.pb({mx1, x, (int)v});
        }
        for(auto& [u, v, w] : now) {
            fw.update(u, v, w);
