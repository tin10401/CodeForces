...
void solve() {
    ...
    for(int i = 0; i < n; i++) {
        for(int j = 0; j < n; j++) {
            int x; cin >> x;
            x ^= x != 2 && (i + j) % 2;
            if(x == 0) graph.add_directional_edge(src, get_hash(i, j), inf);
            else if(x == 1) graph.add_directional_edge(get_hash(i, j), sink, inf);
            if(i + 1 < n) graph.add_bidirectional_edge(get_hash(i, j), get_hash(i + 1, j), 1);
            if(j + 1 < n) graph.add_bidirectional_edge(get_hash(i, j), get_hash(i, j + 1), 1);
        }
    }
    cout << n * (n - 1) * 2 - graph.flow(src, sink) << endl;
}
