...
void solve()
{
    int n; cin >> n;
    string s; cin >> s;
    auto dfs = [&](int left, int right, char ch, auto& dfs) -> int
    {
        if(left == right) return s[left] != ch;
        int middle = left + (right - left) / 2;
        int leftCount = dfs(left, middle, ch + 1, dfs);
        leftCount += (right - middle) - count(s.begin() + middle + 1, s.begin() + right + 1, ch);
        int rightCount = dfs(middle + 1, right, ch + 1, dfs);
        rightCount += middle - left + 1 - count(s.begin() + left, s.begin() + middle + 1, ch);
        return min(rightCount, leftCount);
    };

    cout << dfs(0, n - 1, 'a', dfs) << endl;
}
