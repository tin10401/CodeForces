...
struct CD { // centroid_decomposition
    ...
    void cal(int node, int par, vpll& now, int depth = 1, ll g = 0) {
        now.pb({g, depth});
        ans = max(ans, depth * g);
        for(auto& [nei, w] : graph[node]) {
            if(vis[nei] || nei == par) continue;
            cal(nei, node, now, depth + 1, gcd(g, w));
        }
    }
    ...
    int init(int root = 0, int par = -1) {
        ...
        for(auto& [nei, w] : graph[root]) {
            if(nei == par || vis[nei]) continue;
            curr.emplace_back();
            cal(nei, root, curr.back(), 1, w);
            mx = max(mx, (int)curr.back().size());
            auto& v = curr.back();
            srt(v);
            vpll other;
            const int N = v.size();
        ...
        }
        {
            ...
            for(auto& it : curr) {
                for(auto& [g, len] : it) {
                    if(g * (len + mx) <= ans) continue;
                    for(int c = 1; c <= len; c++) {
                        ans = max(ans, g * (len + mp[g * c]));
                    }
                } 
                for(auto& [g, len] : it) {
                    mp[g] = max(mp[g], len);
