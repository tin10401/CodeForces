...
void solve() {
    ...
    const int K = 11;
    const int N = 4;
    vt<vt<vt<FW<int>>>> root(N, vt<vt<FW<int>>>(K, vt<FW<int>>(K, FW<int>(n + 1, 0))));
    ...
    auto update_at = [&](int i, int delta) {
        int j = get_id(s[i]); 
        for(int k = 1; k < K; k++) {
            root[j][k][i % k].update(i, delta);
        }
    };
    for(int i = 1; i < n; i++) update_at(i, 1);
    int q; cin >> q;
    while(q--) {
        int op; cin >> op;
        if(op == 1) {
        ...
        for(int i = 0; i < len; i++) {
            int p = (i + l) % len;
            int j = get_id(t[i]);
            auto& curr = root[j][len][p];
            res += curr.queries(l, r);
        }
        cout << res << endl;
    }
}
