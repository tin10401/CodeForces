...
struct EulerianPath {
    ...
    EulerianPath(int _nodes, bool _directed = false)
      : nodes(_nodes), directed(_directed), graph(_nodes) {
        if(directed) indeg.assign(nodes,0), outdeg.assign(nodes,0);
        else deg.assign(nodes,0);
    }

    void add_edge(int u, int v, int p, int id) {
        graph[u].pb({v, p, id});
        edges++;
        if(directed) {
            outdeg[u]++;
    ...
    int get_path() {
        ...
        if((int)ans_edges.size() != edges) return false;
        rev(ans_nodes);
        debug(ans_nodes);
        return true;
    }
