...
void solve() {
    ...
    var(3) ans;
    auto dfs = [&](auto& dfs, int row, int col, bool ok = false) -> void {    
        ans.pb({0, row + 1, col + 1});  
        grid[row][col] = '#';
        for(auto& it : dirs) {  
        ...
        if(ok) {    
            ans.pb({1, row + 1, col + 1});  
            ans.pb({2, row + 1, col + 1});
        }
    };
    for(int i = 0; i < n; i++) {    
        for(int j = 0; j < m; j++) {    
    ...
    cout << ans.size() << endl;
    for(auto& [op, row, col] : ans) {   
        cout << get(op) << " " << row << " " << col << endl;
    }
}
