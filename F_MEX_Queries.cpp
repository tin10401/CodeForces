...
class SGT { 
    ...
    void applyInv(int i) {
        root[i].total = root[i].range - root[i].total;
        root[i].inv ^= 1;
    }

	void apply(int i, int val) {
        root[i].total = root[i].range * val;
        lazy[i] = val;
        root[i].inv = 0;
    }
    ...
    int go(iterator) {
        ...
        int middle = midPoint;
        if(root[lc].total == root[lc].range) return go(rp);
        return go(lp);
    }

	int mex() {
        return go(entireTree);
	}
