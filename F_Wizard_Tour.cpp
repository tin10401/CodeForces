...
void solve() {
    ...
    vi vis(n + 1), used(m);
    var(3) ans;
    auto dfs = [&](auto& dfs, int node, int par = -1, int last = -1) -> void {
        vis[node] = true;
        int prev = -1, prev_id = -1;
        for(auto& [nei, id] : graph[node]) {
            if(nei == par) continue;
            if(!vis[nei]) dfs(dfs, nei, node, id);
            if(prev_id != -1 && used[prev_id]) {
                prev_id = prev = -1;
            }
            if(used[id] || prev == nei) continue;
            if(prev == -1) prev = nei, prev_id = id;
            else {
                assert(!used[id] && !used[prev_id]);
                ans.pb({prev, node, nei});
                used[id] = used[prev_id] = true;
                prev_id = prev = -1;
            }
        }
        if(prev != -1 && last != -1 && prev != par && !used[last] && !used[prev_id]) {
            used[last] = used[prev_id] = true;
            ans.pb({par, node, prev});
        }
    }; 
    for(int i = 1; i <= n; i++) {
        if(!vis[i]) dfs(dfs, i);
