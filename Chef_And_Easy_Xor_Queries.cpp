...
struct square_root_decomp {
    ...
    vi sm;
    int n;
    square_root_decomp(const vi& a) : a(a), n(a.size()) {
        block_size = sqrt(n) + 1;
        blocks.rsz(block_size + 10);
        sm.rsz(block_size + 10);
    ...
    int get(int r, ll k) {
        ...
        for(int i = 0; i < id(r); i++) {
            ...
            res += int(ub(all(curr), prefix ^ k) - lb(all(curr), prefix ^ k));
            prefix ^= sm[i];
        }
