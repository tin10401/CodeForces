...
void solve() {
    // in one move choose [x, y] and toggle [[x, y], [x + 1, y - 1], [x, y + 1], [x + 1, y]]
    // looking at the x-axis, we can isolate the x-coordinate out
    // now for the y, looking at the formula of sum x, y, it becomes
    // [x + y], [x + 1 + y - 1], [x + y + 1], [x + 1 + y]
    // [x + y], [x + y], [x + y + 1], [x + y + 1]
    // by using xor isolated_bit, we then get the isolated [x + y] out
    int n; cin >> n;
    ll X = 0, Y = 0;
    while(n--) {
        ll x, y; cin >> x >> y;
