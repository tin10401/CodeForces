...
public: 
    ...
    DSU(int n) {    
        this->n = n;    
        comp = n;
        root.rsz(n, -1), rank.rsz(n, 1), col.rsz(n, 0), s.rsz(n);
        is_bipartite = true;
    }
    ...
    int check(int u, int v) {
        return same(u, v) || s[find(u)].count(v);
    }
};
...
void solve() {
    ...
    map<pii, int> graph;
    DSU root(n);
    auto add_edge = [&](int u, int v, int z) -> void {
        root.ins(u, v);
        root.ins(v, u);
        if(graph.count({u, z})) {
            root.merge(v, graph[{u, z}]);
        }
        if(graph.count({v, z})) {
            root.merge(u, graph[{v, z}]);
        }
        graph[{u, z}] = v;
        graph[{v, z}] = u;
    };
