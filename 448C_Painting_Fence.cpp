...
{   
    ...
    pii queries(int start, int end) 
    {   
        return queries(entireTree, start, end); 
    }   
        
    pii queries(iterator, int start, int end)   
    {   
        if(left > end || start > right) return max_pair; 
        if(left >= start && right <= end) return root[i];   
        int middle = midPoint;  
        return min(queries(lp, start, end), queries(rp, start, end));   
    }   
};
...
{   
    ...
    auto [mn, id] = root.queries(left, right);
    return min(right - left + 1, mn - prev + dfs(left, id - 1, mn) + dfs(id + 1, right, mn));
}
