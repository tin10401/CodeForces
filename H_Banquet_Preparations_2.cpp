...
void solve() {
    ...
    map<int, vi> mp;
    for(int i = 0; i < n; i++) {
        auto& [fish, meat, lim] = a[i]; cin >> fish >> meat >> lim;
        mp[fish + meat - lim].pb(i);
    }
    vi ans(n);
    int res = 0;
    for(auto& [_, it] : mp) {
        var(3) now;
        for(auto& i : it) {
            auto& [fish, meat, lim] = a[i];
            int left = max(0, fish - lim);
            int right = fish - max(0, lim - meat);
            now.pb({left, right, i});
        }
        srt(now);
        min_heap<pii> q;
        for(auto& [l, r, i] : now) {
            if(!q.empty() && q.top().ff < l) {
                res++;
                int target = q.top().ff;
                while(!q.empty()) {
                    auto [_, id] = q.top(); q.pop();
                    auto& [fish, meat, lim ] = a[id];
                    fish -= target;
                    meat = lim - fish;
                }
            }
