...
void solve()
{
    int m, duration, candle; cin >> m >> duration >> candle;    
    vi arr(m);  
    for(auto& it : arr) cin >> it;
    if(candle > duration) {cout << -1 << endl; return;} 
    pq<int, vi, greater<int>> minHeap;  
    int res = 0;
    for(auto& it : arr) 
    {   
        while(!minHeap.empty() && minHeap.top() < it) minHeap.pop();    
        while(minHeap.size() < candle)  
        {   
            minHeap.push(--it + duration);   
            res++;  
        }   
    }
    cout << res << endl;
