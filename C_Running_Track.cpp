...
void solve() {
    ...
    auto getForward = [&](int x) -> pii {    
        int mx = 0; 
        pii ans = {-1, -1};
        for(int i = 0; i < n; i++) {    
            int curr = 0;   
            while(i + curr < n && x + curr < m && s[i + curr] == t[x + curr]) curr++;   
            if(curr > mx) { 
                mx = curr;  
                ans = {i, i + curr - 1};
            }
    ...
    auto getBackward = [&](int x) -> pii {  
        int mx = 0; 
        pii ans = {-1, -1};    
        for(int i = n - 1; i >= 0; i--) {   
            int curr = 0;   
            while(i - curr >= 0 && x + curr < m && s[i - curr] == t[x + curr]) curr++;  
            if(curr > mx) { 
                mx = curr;  
                ans = {i, i - curr + 1};
            }
    ...
    for(int i = 0; i < m;) {    
        ...
        auto& a = res.back();
        auto p = getForward(i); 
        auto g = getBackward(i);    
        if(p.ff == -1) a = g;   
        else if(g.ff == -1) a = p;  
        else if(abs(p.ff - p.ss) > abs(g.ff - g.ss)) a = p; 
        else a = g;
        if(a.ff == -1) {    
            cout << -1 << endl; 
            return;
