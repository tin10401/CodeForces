...
void solve() {  
    ...
    auto get = [&](int row, int col, int len) { 
        return dp[row][col] + dp[row - len][col - len] - dp[row - len][col] - dp[row][col - len];
    };
    int res = 0;    
    for(int i = 1; i <= n; i++) {   
        for(int j = 1; j <= m; j++) {   
            int left = 1, right = min(i, j), ans = -1;  
            while(left <= right) {  
                int middle = midPoint;  
                if(get(i, j, middle) >= k) ans = middle, right = middle - 1;
                else left = middle + 1;
            }
            if(ans != -1) { 
                res += min(i, j) - ans + 1;
