...
void solve() {
    // can change at most one value from the path from root to that vertex, what is the maximum value of that vertex? gcd of the path
    ...
    auto update = [&](int x, int v) -> void {
        for(auto& it : DIV[x]) curr[it] += v;
    };
    auto dfs = [&](auto& dfs, int i = 0, int par = -1, int g = 0, int c = 0) -> void {
        update(a[i], 1);
        int v = g;
        for(auto& j : DIV[a[i]]) {
            if(curr[j] >= c) v = max(v, j);
        }
        ans[i] = v;
        for(auto& nei : graph[i]) {
            if(nei == par) continue;
            dfs(dfs, nei, i, gcd(g, a[i]), c + 1);
        }
        update(a[i], -1);
    };
    dfs(dfs);
