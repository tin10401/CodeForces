...
void solve() {
    ...
    ll res = 0;
    for(int r = 1; r <= n; r++) {
        root.update_range(0, r - 1, -a[r]); // for each segment have the right_endpoint ending before r, it will cost -a[r] to them
        for(auto& [l, c] : ending[r]) { // if you choose to take this segment, since every cost of -a[l - r] was added to the you now add the benefit of c to all segments ending before l
                                        // to tell that it cost a[l, r] and gain c
            root.update_range(0, l - 1, c);
        }
        res = max(res, root.get()); // either skip or take, that's why max(res, root.get())
        root.update_at(r, res);
    }
    cout << res << endl;
}
