...
{   
    ...
    {   
        ...
        for(auto& ch : s)   
        {   
            if(!curr->children[ch - '0']) curr->children[ch - '0'] = newNode(ch - '0'); 
    ...
    int dfs(Node* curr, bool seen, int cnt) 
    {   
        seen |= curr->data == 0;    
        cnt += curr->data == 0; 
        int res = seen ? cnt - 1 : 0;   
        for(int i = 0; i < 2; i++)  
        {   
            if(curr->children[i])   
            {   
                res = (res + dfs(curr->children[i], seen, cnt)) % MOD;  
            }   
        }   
        return res; 
    }   
