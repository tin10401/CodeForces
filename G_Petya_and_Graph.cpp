...
void solve() {
    ...
    for(int i = 0; i < n; i++) {
        int x; cin >> x;
        graph.add_directional_edge(i + m, sink, x);
    }
    ll s = 0;
    for(int i = 0; i < m; i++) {
        int u, v, w; cin >> u >> v >> w;
        u--, v--;
        s += w;
        graph.add_directional_edge(src, i, w);
        graph.add_directional_edge(i, u + m, INF);
        graph.add_directional_edge(i, v + m, INF);
    }
    cout << s - graph.flow(src, sink) << '\n';
