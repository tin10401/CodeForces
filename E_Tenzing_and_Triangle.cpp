...
void solve() {
    ...
    for(int i = 1; i <= k; i++) {
        for(auto& [j, c] : pos[i]) {
            root.update_range(0, j, c); // it cost c to go from anywhere from 0 to j that cost c extra
        }
        dp = max(dp, -A * i + root.get()); // since any points gets added by c, if A * i is less than some of them, it's better to go there
        debug(pos[i], dp, i, A * i);
        root.update_at(i, dp + A * i); // the actual value at this current points
    }
    debug(dp);
    cout << total - dp << endl;
}
