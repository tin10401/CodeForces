...
void solve() {
    ...
    for(int c = 1; c <= k; c++) {
        ...
        int lca = color[c][0];
        for(auto& u : color[c]) lca = g.lca(lca, u);
        for(auto& u : color[c]) {
            a[u] = 0;
            for(int v = u; v != lca; v = g.parent[v]) {
                if(a[v] && a[v] != c) {
                    cout << 0 << endl;
                    return;
                }
    ...
    using info = array<mint, 2>;
    auto dfs = [&](auto& dfs, int node) -> info {
        info res = {};
        res[node <= k] = 1;
        for(auto& nei : graph[node]) {
            auto [f, s] = dfs(dfs, nei);
            auto way = f + s;
            auto new_unmarked = res[0] * way;
            auto case1 = res[0] * s;
            auto case2 = res[1] * s;
            auto case3 = res[1] * f;
            auto new_marked = case1 + case2 + case3;
            res[0] = new_unmarked;
            res[1] = new_marked;
        }
        return res;
    };
    auto res = dfs(dfs, a[1]);
    cout << res[1] << endl;
}
