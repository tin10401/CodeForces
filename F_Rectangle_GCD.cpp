...
void solve() {
    int n, q; cin >> n >> q;
    vi a(n), b(n); cin >> a >> b;
    vi diff_a(n), diff_b(n);
    for(int i = 1; i < n; i++) {
        diff_a[i] = abs(a[i] - a[i - 1]);
        diff_b[i] = abs(b[i] - b[i - 1]);
    }
    SparseTable<int> ta(diff_a, [](const int& a, const int& b) {return gcd(a, b);});
    SparseTable<int> tb(diff_b, [](const int& a, const int& b) {return gcd(a, b);});
    while(q--) {
        int r1, r2, c1, c2; cin >> r1 >> r2 >> c1 >> c2;
        r1--, r2--, c1--, c2--;
        int res = gcd(gcd(a[r1] + b[c1], ta.query(r1 + 1, r2)), tb.query(c1 + 1, c2));
        cout << res << '\n';
    }
