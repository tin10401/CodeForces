...
void solve() {
    ...
    a.pb({x, y});
    auto get_score = [&](int i, int j) -> ll {
        auto get = [&](int ii, int jj) -> int {
            int dx = abs(a[ii].ff - a[jj].ff);
            int dy = abs(a[ii].ss - a[jj].ss);
            return dx * dx + dy * dy;
        };
        return get(n, i) + get(i, j) + get(j, n);
    };
    mset(dp, -1);
    int total = (1LL << n) - 1;
    auto dfs = [&](auto& dfs, int mask = 0) -> int {
        if(mask == 0) return 0;
        auto& res = dp[mask];
        if(res != -1) return res;
        int i = min_bit(mask);
        int new_mask = mask ^ (1LL << i);
        res = dfs(dfs, new_mask) + get_score(i, i);
        choose[mask] = {i, -1};
        for(int j = i + 1; j < n; j++) {
            if(!have_bit(mask, j)) continue;
            auto now = get_score(i, j) + dfs(dfs, new_mask ^ (1 << j)); 
            if(now < res) {
                res = now;
                choose[mask] = MP(i, j);
            }
