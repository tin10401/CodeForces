...
class SGT { 
	...
	int get() {
        ...
        int res = 0;
        for(int i = 0; i < 2; i++) {
            for(int j = 0; j < 2; j++) res = max(res, curr[1][i][j]);
        }
        return res;
	}
    ...
    T merge(const T &left, const T &right) {
        ...
        for (int a = 0; a < 2; ++a) {
            for (int b = 0; b < (a ? 1 : 2); ++b) {
                ...
                auto &L = left.dp[a];
                auto &R = right.dp[b];
                curr[0][0] = max(curr[0][0], L[0][0] + R[0][0]);
                curr[0][0] = max(curr[0][0], L[0][0] + R[1][0]);
                curr[0][0] = max(curr[0][0], L[0][1] + R[0][0]);

                curr[0][1] = max(curr[0][1], L[0][0] + R[0][1]);
                curr[0][1] = max(curr[0][1], L[0][0] + R[1][1]);
                curr[0][1] = max(curr[0][1], L[0][1] + R[0][1]);

                curr[1][0] = max(curr[1][0], L[1][0] + R[0][0]);
                curr[1][0] = max(curr[1][0], L[1][0] + R[1][0]);
                curr[1][0] = max(curr[1][0], L[1][1] + R[0][0]);

                curr[1][1] = max(curr[1][1], L[1][0] + R[0][1]);
                curr[1][1] = max(curr[1][1], L[1][0] + R[1][1]);
                curr[1][1] = max(curr[1][1], L[1][1] + R[0][1]);
            }
        }
        return res;
    }
...
void solve() {
    ...
    for(int i = 0; i < n; i++) {
        cin >> a[i].ff;
        a[i].ss = i;
        root.update_at(i, Node(a[i].ff));
    }
    srt(a);
    int res = 0;
    for(auto& [x, i] : a) {
        res = max(res, x + root.get());
        root.update_at(i, Node());
    }
    cout << res << endl;
}
