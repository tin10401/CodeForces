...
void solve() {
    ...
    vt<SparseTable<int>> t(m + 1);
    for(int i = 1; i <= m; i++) {
        t[i] = SparseTable<int>(b[i], [](const int& a, const int& b) {return max(a, b);});
    }
    ll res = 0;
    auto dfs = [&](auto& dfs, int l, int r, int idl, int idr) -> void {
        ...
        int middle = (l + r) >> 1;
        ll best = -INF, nxt = -1;
        for(int i = idl; i <= min(middle, idr); i++) {
            ll now = 0;
            for(int b = 1; b <= m; b++) {
                now += t[b].query(i, middle);
            }
            now -= prefix[middle - 1] - prefix[i - 1];
            if(now > best) {
                best = now;
                nxt = i;
            }
