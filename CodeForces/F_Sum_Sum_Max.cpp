...
void solve() {
    ...
    for(int i = 1; i <= n; i++) {
        auto& [x, y] = a[i];
        if(i == 1) res = x;
        block[i] = block[i - 1] + x * y;
    }
    ll total = 0;
    for(int i = 1; i <= n; i++) {
        auto& [x, y] = a[i];
        ll left = 0, right = y, now = 0;
        while(left <= right) {
            ll middle = midPoint;
            if(block[i - 1] + middle * x >= 0) now = middle, left = middle + 1;
            else right = middle - 1;
        }
        auto get = [&](ll k) -> ll {
            return block[i - 1] * k + k * (k + 1) / 2 * a[i].ff;
        };
        if((i == 1 && now > 0) || i > 1) {
            res = max(res, total + get(now));
        }
        total += get(y);
        res = max(res, total);
    }
