...
{   
    ...
    {   
        ...
        {   
            int bits = (num >> i) & 1;  
            int lim_bits = (lim >> i) & 1;  
            int x = bits ^ lim_bits;    
            if(lim_bits && curr->children[bits]) res += curr->children[bits]->cnt;   
            if(!curr->children[x]) break;   
...
void solve()
{
    ...
    {   
        int num; cin >> num;
        res += trie.search(num, r + 1) - trie.search(num, l);
        trie.insert(num);
    }   
    cout << res << endl;
}
