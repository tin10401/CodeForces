...
void solve() {
    ...
    vvi a(n, vi(m)); cin >> a;
    auto b(a);
    TWO_DIMENSIONAL_RANGE_QUERY prefix(b);
    for(int i = 0; i < n; i++) {
        for(int j = 0; j < m; j++) {
            b[i][j] = a[i][j] * (j + 1);
        }
    ...
    TWO_DIMENSIONAL_RANGE_QUERY factor(b);
    while(q--) {
        ll r1, c1, r2, c2; cin >> r1 >> c1 >> r2 >> c2;
        ll normal = prefix.get(r1, c1, r2, c2);
        ll large = PREFIX.get(r1, c1, r2, c2);
        ll f = factor.get(r1, c1, r2, c2);
        large -= (c1 - 1) * normal;
        f -= (r1 - 1) * normal;
        f *= (c2 - c1 + 1);
        large += f;
