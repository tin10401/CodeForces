...
void solve() {
    ...
    auto dfs = [&](auto& dfs, int mask, int i, int j, int id) -> int {  
        if(id == n) return 0;
        int& res = dp[mask][i][j];  
        if(res != -1) return res;   
        res = INF;  
        for(int k = 0; k < n; k++) {    
            ...
            int newMask = mask ^ (1LL << k);    
            int val = a[i] ^ a[j] ^ a[k];
            res = min(res, val * b[id] + dfs(dfs, newMask, j, k, id + 1));
        }
        return res;
    };
    int ans = INF;  
    for(int i = 0; i < n; i++) {    
        for(int j = 0; j < n; j++) {    
            if(i == j) continue;    
            ans = min(ans, dfs(dfs, (1LL << i) | (1LL << j), i, j, 2));
        }
    }
    cout << ans << endl;
