...
using info = array<vi, 3>;
map<info, int> dp[2];
void solve() {
    int n, m, k; cin >> n >> m >> k;
    vi a(n), b(m), c(k); cin >> a >> b >> c;
    srt(a), srt(b), srt(c);
    auto dfs = [&](auto& dfs, const info& state, int turn = false) -> int {
        if(dp[turn].count(state)) return dp[turn][state]; 
        auto& res = dp[turn][state];
        auto [aoki, taka, table] = state;
        int N = taka.size();
        for(int i = 0; i < N; i++) {
            auto next_taka = taka;
            auto next_table = table;
            next_table.pb(next_taka[i]);
            next_taka.erase(next_taka.begin() + i);
            srt(next_table);
            info s = {next_taka, aoki, next_table};
            if(dfs(dfs, s, !turn) == turn) return res = turn;
            const int M = next_table.size();
            for(int j = 0; j < M; j++) {
                if(next_table[j] < taka[i]) {
                    auto take_taka = next_taka;
                    auto take_table = next_table;
                    take_taka.pb(take_table[j]);
                    take_table.erase(take_table.begin() + j);
                    srt(take_taka);
                    s = {take_taka, aoki, take_table};
                    if(dfs(dfs, s, !turn) == turn) return res = turn;
                }
            }
        }
