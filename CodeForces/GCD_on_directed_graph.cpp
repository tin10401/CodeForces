...
void solve() {
    ...
    g.generate();
    auto graph = g.condense();
    int N = graph.size();
    vi value(N);
    for(int i = 0; i < n; i++) {
        value[g.comp[i]] = gcd(value[g.comp[i]], a[i]);
    }
    vt<set<int>> s(N);
    queue<int> q;
    vi vis(N);
    ...
    while(!q.empty()) {
        ...
        for(auto& nei : graph[node]) {
            if(vis[nei]) continue;
            for(auto& x : s[node]) {
                s[nei].insert(gcd(value[nei], x));
            }
            if(--g.degree[nei] == 0) q.push(nei), vis[nei] = true;
        }
    }
    int res = MIN(a);
    for(int i = 0; i < N; i++) res = min(res, *s[i].begin());
    cout << res << endl;
}
