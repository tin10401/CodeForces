...
class SGT { 
    ...
    void update(iter, int id, ll val) {  
        if(left == right) { 
            root[i].ff += val * left;
            root[i].ss += val;
            return;
    ...
    ll queries(iter, ll k) {
        if(root[i].ss <= k) return root[i].ff;
        if(left == right) return k * left;
        int middle = midPoint;
        if(root[lc].ss <= k) {
            return root[lc].ff + queries(rp, k - root[lc].ss);
        }
        return queries(lp, k);
    }
	
	ll get(ll k) {
        return queries(entireTree, k);
	}
