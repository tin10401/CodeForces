...
void solve() {
    ...
    vll a(n); cin >> a;
    ll res = 0;
    auto dfs = [&](auto& dfs, int node = 0, ll path = 0) -> ll {
        res += a[node] * path;
        const int N = graph[node].size();
        if(N == 0) return a[node];
        vll now;
        // at any points, you want to distribute them evenly
        // what's leftover is extra = path % N
        // you're allow to take this many extra value
        // and the one that's next biggest is the now[extra] because everything before it already got distributed once
        // so it'll be the next candidate
        for(auto& nei : graph[node]) {
            now.pb(dfs(dfs, nei, path / N));
        }
        srtR(now);
        int extra = path % N;
