...
void solve() {
    ...
    vi arr(n); cin >> arr;  
    int res = 0;
    vt<bitset<MX>> suffix(n + 1);   
    suffix[n].set(0);
    for(int i = n - 1; i >= 0; i--) {   
        suffix[i] = (suffix[i + 1] | (suffix[i + 1] << arr[i]));
    }
    bitset<MX> s;
    s.set(0);
    for(int i = 0; i < n; i++) {    
        int left = fmax(0, k - arr[i]);
        vi a;
        bool ok = true;
        ...
        for(int j = 0; j < k && ok; j++) {    
            if(s[j]) {  
                if(j >= left) { 
                    ...
                    auto it = lb(all(a), left - j);
                    if(it != end(a) && *it + j <= k - 1) ok = false;
                }        
            }
        }
        res += ok;
