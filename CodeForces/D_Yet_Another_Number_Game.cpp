...
void solve() {
    ...
    if(n == 3) {    
        cout << ((arr[0] ^ arr[1] ^ arr[2]) ? A : B) << endl; 
        return;
    }
    mset(dp, -1);
    auto dfs = [&](auto& dfs, int x, int y, bool turn) -> int { 
        if(x == 0 && y == 0) return !turn;  
        int& res = dp[x][y][turn];  
        if(res != -1) return res;   
        for(int i = 1; i <= max(x, y); i++) {   
            if(i <= x) {    
                if(dfs(dfs, x - i, y, !turn) == turn) return res = turn;
            }
            if(i <= y) {    
                if(dfs(dfs, x, y - i, !turn) == turn) return res = turn;
            }
        }
        for(int i = 1; i <= min(x, y); i++) {   
            if(dfs(dfs, x - i, y - i, !turn) == turn) return res = turn;
        }
        return res = !turn;
    };
    int ans = dfs(dfs, arr[0], arr[1], true);   
    cout << (ans ? A : B) << endl;
}
