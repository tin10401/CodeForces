...
void solve() {
    ...
    ll res = 0;
    if(red.size() > 1) res += red.back() - red.front();
    if(blue.size() > 1) res += blue.back() - blue.front();
    const int N = p.size();
    for(int i = 0; i + 1 < N; i++) {
        int l = p[i], r = p[i + 1];
        auto compute = [&](const vi& a) -> int {
            int mx = 0;
            for(auto it = lb(all(a), l); *it != r; it++) {
                assert(next(it) != end(a));
                mx = max(mx, *next(it) - *it);
            }
            return mx;
        };
        ll len = r - l;
        ll t = (len - compute(blue)) + (len - compute(red)) + len; // cut the mx of both of them and use the len as a bridge
        res = min(res, res - 2 * len + t);
