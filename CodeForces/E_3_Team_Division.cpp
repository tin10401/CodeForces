...
void solve() {
    ...
    for(int ii = 0; ii < n; ii++) {
        int c1 = a[ii] != 1, c2 = a[ii] != 2, c3 = a[ii] != 3;
        int x = b[ii];
        s += x;
        for(int i = target; i >= 0; i--) {
            for(int j = target; j >= 0; j--) {
                if(i + x <= target) dp[i + x][j] = min(dp[i + x][j], dp[i][j] + c1);
                if(j + x <= target) dp[i][j + x] = min(dp[i][j + x], dp[i][j] + c2);
                if(s - (i + j) <= target) dp[i][j] += c3;
            }
        }
    }
    int res = dp[target][target];
    cout << (res >= inf ? -1 : res) << '\n';
}
