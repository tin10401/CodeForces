...
void solve() {
    ...
    for(int i = 0; i < m; i++) {
        for(int j = 0, r = 0; j < n; j++) {
            while(r < n && (ll)(a[r].ff) * (r - j + 1) < b[i]) r++;
            nearest[i][j] = r;
        }
    }
    vi dp(1LL << m, n + 1), par(1LL << m, -1);
    dp[0] = 0;
    ...
    vvi ans(m);
    for(int mask = (1LL << m) - 1; mask; mask = par[mask]) {
        int id = log2(mask ^ par[mask]);
        int l = dp[par[mask]], r = dp[mask] - 1;
        for(int j = l; j <= r; j++) {
            ans[id].pb(a[j].ss);
        }
    }
