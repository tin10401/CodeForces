...
void solve() {
    // minimum moves to sorted this
    // cost 0 if same color otherwise cost 1
    // answer is total # of inversion - total # of inversion in each group of same color
    int n; cin >> n;
    vi c(n); cin >> c;
    vvi a(n + 1);
    for(int i = 0; i < n; i++) {
    ...
    for(int i = 0; i <= n; i++) {
        ...
        int N = curr.size();
        for(auto& x : curr) {
            int t = root.queries_range(x + 1, n);
            if(i == 0) res += t;
            else res -= t;
            root.update(x, 1);
        }
