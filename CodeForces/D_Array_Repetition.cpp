...
void solve() {
    ...
    for(int i = 1; i <= n; i++) {    
        cin >> op[i] >> v[i];   
        if(op[i] == 1) {    
            length[i] = min(INF, length[i - 1] + 1);
        }
        else {  
            if(length[i - 1] > INF / v[i]) {   
                length[i] = INF;
            }
            else {  
                length[i] = length[i - 1] * (v[i]);
            }
        }
    }
    auto f = [&](auto& f, ll pos) -> int {  
        int id = lb(all(length), pos) - begin(length);  
        if(op[id] == 1) return v[id];   
        return f(f, (pos - 1) % length[id - 1] + 1);
    };
    for(int i = 0; i < m; i++) {    
        ll p; cin >> p;    
        cout << f(f, p) << (i == m - 1 ? '\n' : ' ');
