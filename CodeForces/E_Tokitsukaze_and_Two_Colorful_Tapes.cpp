...
void solve() {
    ...
    vi color(n + 1);
    int l = 1, r = n;
    auto dfs2 = [&](auto& self, int node, int par = -1, int t = 0, bool ok = true) -> void {
        vis[node] = true;
        if(!ok) color[node] = t == 0 ? l++ : r--;
        else color[node] = (n + 1) / 2;
        t ^= 1;
        for(auto& nei : graph[node]) {
            if(vis[nei]) continue;
            self(self, nei, node, t, false);
        }
    };
    fill(all(vis), 0); 
    while(!q.empty()) {
        auto [u, sz] = q.front(); q.pop();
        dfs2(dfs2, u, -1, 0, sz & 1);
    }
    ll res = 0;
    for(int i = 0; i < n; i++) {
        res += abs(color[a[i]] - color[b[i]]);
    }
    cout << res << endl;
