...
void solve() {
    ...
    auto get = [&](int mask, int row, int col) -> bool { 
        if(row >= 0 && col >= 0 && row < n && col < m) {    
            return ((mask & (1LL << (row * m + col))) == 0);
        }
        return false;
    };
    auto ok = [&](int mask, int p) -> bool {    
        if((mask >> p) & 1) return false;
        int row = p / m;    
        int col = p % m;    
        for(auto& it : dirs) {  
            if(get(mask, row + it[0], col + it[1])) return true;
        }
        return false;
    };
    auto dfs = [&](auto& dfs, int mask, bool turn) -> int {    
        int& res = dp[mask][turn];
        if(res != -1) return res;   
        for(int i = 0; i < n * m; i++) {    
            if(ok(mask, i)) {   
                int newMask = mask | (1LL << i);    
                if(dfs(dfs, newMask, !turn) == turn) return res = turn;
            }
        }
        return res = !turn;
    };
    while(q--) {    
        cout << dfs(dfs, getMask(), false) << endl;
    }
}
