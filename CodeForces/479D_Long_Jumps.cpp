...
void solve()
{
    ...
    {   
        int val; cin >> val;
        if(s.count(val - a)) ok[0] = true;   
        if(s.count(val - b)) ok[1] = true;   
        if(s.count(val - (b - a)) && (val - b >= 0 || val + a <= l))   
        {   
            ok[2] = true;   
            res = b;
        }   
        if(s.count(val - (a + b)) && (val - b >= 0 || val - a >= 0))
        {   
            ok[3] = true;   
            res = b;
        }   
    ...
    else if(ok[0]) cout << 1 << endl << b << endl;  
    else if(ok[1]) cout << 1 << endl << a << endl;  
    else if(ok[2] || ok[3]) cout << 1 << endl << res << endl;   
