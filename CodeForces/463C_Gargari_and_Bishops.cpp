...
int diagnal[MX], anti_diagnal[MX];
void solve()
{
    int n; cin >> n;
    vvi grid(n, vi(n));
    ...
    {   
        for(int j = 0; j < n; j++)  
        {   
            int& val = grid[i][j]; cin >> val;
            anti_diagnal[i + j] += val; 
            diagnal[i - j + n] += val;  
        }   
    }   
    var(3) ans(2, {INT_MIN, 0, 0});  
    for(int i = 0; i < n; i++)  
    {   
        for(int j = 0; j < n; j++)  
        {   
            auto& [mx, x, y] = ans[(i + j) & 1];    
            int sm = diagnal[i - j + n] + anti_diagnal[i + j] - grid[i][j];
            if(sm > mx) 
            {   
                mx = sm, x = i + 1, y = j + 1;  
            }   
