...
void solve() {
    ...
    vpii a(k); cin >> a;
    for(auto& [x, y] : a) x--, y--;
    vi roads(m); cin >> roads;
    int q; cin >> q;
    vpii query(q);
    for(int i = 0; i < q; i++) {
        auto& [x, y] = query[i]; cin >> x >> y;
        x--;
        roads[x] -= y;
    }
    const int N = n + m;
    int src = N, sink = N + 1;
    dinic<ll> graph(N + 2);
    for(int i = 0; i < n; i++) graph.add_directional_edge(src, i, 1);
    vi id;
    for(int i = 0; i < m; i++) {
        id.pb(graph.add_directional_edge(i + n, sink, max(0, roads[i])));
    }
    for(auto& [x, y] : a) graph.add_directional_edge(x, y + n, 1);
    ll ans = graph.flow(src, sink);
    vll res(q);
    for(int i = q - 1; i >= 0; i--) {
        auto& [x, y] = query[i];
        graph.adj[x + n][id[x]].capacity += y;
        ans += graph.flow(src, sink);
        res[i] = ans;
    }
