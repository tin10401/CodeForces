...
void solve() {
    ...
    for(int i = 1; i <= n; i++) {
        if(p <= m && s[i] == t[p]) p++;
        for(int j = 0; j < p; j++) {
            res = min(res, n - lcp[i + 1][j + 1] + i - j + 1);
            // 1 is moving it to the beginning
            // it cost suffix + 2 * i - j 
            // j here means that at any index that it is the corrent index of s[i] == t[i]
            // it immediately delete the thing before it
            // so instead of 2 * i, it becomes 2 * i - j
            // looks like gap in the prefix
            // you cannot go over p 
        }
    }
    cout << (p <= m ? -1 : res) << endl;
}
