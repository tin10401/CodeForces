...
void solve() {
    int n; cin >> n;
    vpii a(n); cin >> a;
    sort(all(a), [](const pii& a, const pii& b) {return a.ff + a.ss < b.ff + b.ss;}); // imagine different circle and we want to determine the maximum 
                                                                                      // amount of non-intersecting circle
    int res = 1, last = 0;
    for(int i = 1; i < n; i++) {
        if(abs(a[i].ff - a[last].ff) >= a[i].ss + a[last].ss) {
            res++;
            last = i;
        }
    }
