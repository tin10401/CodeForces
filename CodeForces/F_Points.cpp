...
struct info {
    ll count, sm, total;
    info(ll count = 0, ll sm = 0, ll total = 0) : count(count), sm(sm), total(total) {}
};
...
void solve() {
    FW<int> occur(MX, 0, [](const int& a, const int& b) {return a + b;});
    SGT<info> tree(MX, info(), [](const info& a, const info& b) { return info(a.count + b.count, a.sm + b.sm, a.total + b.total);}, 
                               [&](int i, int left, int right, ll delta) {
                                    auto& now = tree.root;
                                    now[i].total += delta * now[i].sm + delta * (delta - 1) / 2 * now[i].count;
                                    now[i].sm += delta * now[i].count;
                                    tree.lazy[i] += delta; // value is 3, count is 1, delta is 2
                                                           // add delta * sm as a base
                                                           // then anything came after is count * delta * (delta - 1) / 2
                               });
    int q, d; cin >> q >> d;
    vi exist(MX);
    while(q--) {
        int x; cin >> x;
        if(!exist[x]) {
            tree.update_range(max(0, x - d), x, 1);
            ll cnt = occur.queries_range(x, min(x + d, MX - 1));
            tree.update_at(x, info(1, cnt, cnt * (cnt - 1) / 2));
            occur.update(x, 1);
        }
        else {
            tree.update_range(max(0, x - d), x, -1);
            tree.update_at(x, info());
            occur.update(x, -1);
        }
        exist[x] ^= 1;
        cout << tree.get().total << endl;
    }
