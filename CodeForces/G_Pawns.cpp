...
void solve() {
    ...
    SGT<int> root(N, 0);
    for(int i = 0; i < N; i++) {
        root.update_at(i, i - n);
    }
    auto f = [&](int x, int y) -> int { 
        return abs(x - k) + y; // return the left_most position where this pawn will land
    };
    set<pii> s;
    multiset<int> pos;
    while(m--) {
        ...
        if(s.count({r, c})) {
            ...
            root.update_range(0, p - 1, -1);
        }
        else {
            s.insert({r, c});
            pos.insert(p);
            root.update_range(0, p - 1, 1); // meaning add this pawn will occupy from this position backward
                                            // so at any time if a pawn before it needs to go up here, it will be charged 1 more
                                            // cause you can always move to the right
        }
        cout << root.queries_range(0, pos.empty() ? 0 : *pos.rbegin() - 1) << endl;
    }
}
