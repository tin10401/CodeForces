...
void solve() {  
    ...
    string vowel = "aeoiu";
    for(int i = 1; i <= m; i++) {    
        cnt[i] += cnt[i - 1] + (vowel.find(s[i - 1]) != string::npos);
    }
    vpii prefix(n + 1, {0, 0});    
    for(int i = 1; i <= n; i++) {    
        int x; cin >> x;    
        int pos = abs(x);   
        if(x >= 0) {    
            prefix[i] = MP(cnt[pos + 1], 0);
        }
        else {  
            prefix[i] = MP(cnt[m] - cnt[pos], pos);
        }
        prefix[i].ff += prefix[i - 1].ff;
    }
    auto compute = [&](int k) -> int { 
        int left = 0, right = n, id = -1;   
        while(left <= right) {  
            int middle = midPoint;  
            if(prefix[middle].ff >= k) id = middle, right = middle - 1;  
            else left = middle + 1;
        }
        auto it = lb(all(prefix), MP(k, 0));
        if(id == -1) return -1;
        k -= prefix[id - 1].ff;
        int pos = prefix[id].ss;
        left = pos, right = m;  
        int res = -1;
        while(left <= right) {  
            int middle = midPoint;  
            if(cnt[middle] - cnt[pos] >= k) res = middle - 1, right = middle - 1;   
            else left = middle + 1;
        }
        return res;
    };
