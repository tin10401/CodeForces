...
void solve() {
    ...
    auto dikstra = [&](int source) -> pair<vi, vi> { 
        vi cnt(n), dp(n, INF);  
        dp[source] = 0; 
        cnt[source] = 1;    
        queue<pii> q;   
    ...
    auto [dp1, cnt1] = dikstra(0);  
    auto [dp2, cnt2] = dikstra(n - 1);
    vvi c(n, vi(n));
    for(int i = 0; i < n; i++) {    
        for(auto& nei : graph[i]) { 
            if(dp1[i] + 1 + dp2[nei] == dp1[n - 1]) {   
                c[i][nei] = c[nei][i] = cnt1[i] * cnt2[nei];
            }
        } 
    }
    ld res = 0;
