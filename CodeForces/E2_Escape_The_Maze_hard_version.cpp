...
void solve() {
    ...
    auto dfs = [&](auto& dfs, int node = 0, int par = -1, int d = 0) -> int {
        int s = 0;
        bool children = true;
        for(auto& nei : graph[node]) {
            if(nei == par) continue;
            int c = dfs(dfs, nei, node, d + 1);
            if(c < 0) children = false;
            else s += c;
            dp[node] = min(dp[node], dp[nei] + 1);
        }
        if(dp[node] <= d) return 1;
        if((graph[node].size() == 1 && node) || !children) return -1;
        return s;
    };
    debug(dp);
    cout << dfs(dfs) << endl;
