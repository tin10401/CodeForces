...
void solve() {
    // count the number of subarray where a[n / 2] == a[(n - 1) / 2]
    // approach : count the number of bad subarray containing x(x <= 10 is constraint) as a median
    ll n; cin >> n;
    vi a(n); cin >> a;
    ll total = n * (n + 1) / 2;
    ll res = 0;
    ...
    for(int x = 1; x <= 10; x++) {
        ...
        srtU(t);
        auto get = [&](int x) -> int {
            return int(lb(all(t), x) - begin(t));
        };
        for(auto& x : prefix) x = get(x);
        vi mp(t.size());
        for(int i = 1, j = 0; i <= n; i++) {
            if(a[i - 1] == x) {
                while(j < i) { // only move when containing at least one x
                    mp[prefix[j]]++;
                    j++;
                }
            }
            res += mp[prefix[i]]; // this means that number <= x == number greater than x -> so x is not a median -> bad
            if(mp[prefix[i]]) {
                debug(i, prefix, mp[prefix[i]], x, res);
            }
        }
    }
    cout << total - res << endl;
