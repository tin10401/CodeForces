...
void solve() {
    ...
    for(int i = 1; i < n; i++) suffix += int(a[i - 1] >= a[i]);
    int res = inf;
    for(int i = 0; i < n; i++) {
        res = min(res, int(i > 0) + prefix + suffix); // split left part to negative and right part positive
        if(i + 1 < n ) suffix -= int(a[i] >= a[i + 1]);
        if(i) prefix += int(a[i - 1] <= a[i]);
    }
    cout << res << endl;
}
