...
void solve() {
    ...
    while(q--) {
        ...
        if(op == 1) {
            int l, r; cin >> l >> r;
            Q.pb({--l, --r, int(update_Q.size()), int(Q.size())});
        }
        else {
            int j, x; cin >> j >> x;
            update_Q.pb({--j, x, -1});
    ...
    vi res(int(Q.size()));
    int l = 0, r = -1, cur_update = 0;
    for (auto &[ql, qr, last_update, idx] : Q) {
        while (cur_update < last_update) {
            auto &[i, x, orig] = update_Q[cur_update++];
            if (l <= i && i <= r) {
                modify(orig, -1);
                modify(x, 1);
            }
