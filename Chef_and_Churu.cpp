...
struct square_root_decomp {
    ...
    square_root_decomp(const vll& a) : n(a.size()), root(n, 0, [](const ull& a, const ull& b) {return a + b;}) {
        block_size = sqrt(n);
        num_blocks = (n + block_size - 1) / block_size;
        debug(a);
        blocks.assign(num_blocks, 0);
        cnt = vvi(num_blocks, vi(n));
        for(int i = 0; i < n; i++) {
            root.update_at(i, a[i]);
        }
        for(int i = 0; i < num_blocks; i++) {
            for(int j = start_id(i * block_size); j <= end_id(i * block_size); j++) {
                cnt[i][l[j]]++;
                if(r[j] + 1 < n) {
                    cnt[i][r[j] + 1]--;
                }
    ...
    ull get(int R) {
        ...
        for (int i = start_id(R); i <= R; i++) {
            res += root.queries_range(l[i], r[i]);
        }
        return res;
    }
    
    ull queries_range(int l, int r) {
        if(l == 0) return get(r);
        return get(r) - get(l - 1);
    }
    
    void update(int pos, ll x) {
        root.update_at(pos, x);
        for(int i = 0; i < num_blocks; i++) {
            blocks[i] += (ll)cnt[i][pos] * x;
        }
    }
};
