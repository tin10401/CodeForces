...
void solve() {
    ...
    vvi left_reach(n + 1, vi(2, 1)), right_reach(n + 1, vi(2, 1));   
    for(int i = 2; i <= n; i++) {   
        int p = arr[i - 1]; 
        left_reach[i][p] = left_reach[i - 1][p] + 1;    
    }
    for(int i = n - 1; i > 0; i--) {    
        int p = !arr[i]; 
        right_reach[i][p] = right_reach[i + 1][p] + 1;  
    }
    debug(left_reach, right_reach);
    right_reach[n][0] = 1;
    int u = 0;  
    int q; cin >> q;    
    while(q--) {    
        ...
        if(op == 'U') u ^= 1;   
            ...
            int i; cin >> i;
            cout << left_reach[i][u] + right_reach[i][u] - 1 << endl;
        }
