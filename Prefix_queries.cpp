...
{   
    ...
    {   
        int bits = (num >> i) & 1;
        if(!t[node][bits]) t[node][bits] = ++nxt;
        node = t[node][bits];
        a[node] += val; 
    }   
}   
...
{   
    ...
    {   
        int bits = (num >> i) & 1;
        if(!t[node][bits] || !a[t[node][bits]]) return 62 - i - 1;  
        node = t[node][bits];   
    }   
    return 62;  
}   

void dfs(int node, int par, vi& val, vvi & graph, vvpii & queries, vi& res)
{
    modify(val[node], 1);
    for(auto& [v, id] : queries[node])
    {
        res[id] = search(v);
    }
    for(auto& nei : graph[node])
    {
        if(nei != par) dfs(nei, node, val, graph, queries, res);
    }
    modify(val[node], -1);
}
