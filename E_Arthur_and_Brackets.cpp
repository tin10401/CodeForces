...
const int N = 602;
int dp[N][N], choices[N][N], L[N], R[N];
void solve() {
    int n; cin >> n;
    for(int i = 0; i < n; i++) {
        cin >> L[i] >> R[i];
    ...
    memset(dp, -1, sizeof(dp));
    {
        auto dfs = [&](auto& dfs, int l, int r) -> int {
            ...
            auto& res = dp[l][r];
            if(res != -1) return res;
            for(int cnt = 0; l + cnt <= r; cnt++) {
                int dist = 2 * cnt + 1;
                if(L[l] <= dist && dist <= R[l]) {
                    if(dfs(dfs, l + 1, l + cnt) && dfs(dfs, l + cnt + 1, r)) {
                        choices[l][r] = cnt;
                        return res = true;
                    }
                }
    ...
    auto dfs = [&](auto dfs, int l, int r) -> string {
        if(l > r) return "";
        int cnt = choices[l][r];
        auto in = dfs(dfs, l + 1, l + cnt);
        auto out = dfs(dfs, l + cnt + 1, r);
        return "(" + in + ")" + out;
    };
    cout << dfs(dfs, 0, n - 1) << '\n';
}
