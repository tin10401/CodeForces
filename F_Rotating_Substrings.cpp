    ...
    for(int i = 1; i <= n; i++) {
        for(int j = 1; j <= n; j++) {
            ...
            if(s[0][i] == s[1][j]) {
                bool ok = true;
                for(int k = 0; k < 26; k++) {
                    ok &= cnt[0][i][k] <= cnt[1][j][k];
                }
                if(ok) dp[i][j] = max(dp[i][j], dp[i - 1][j - 1] + 1); // abc and cba, require at least 3 moves, but since c is a safe spot
                                                                       // it's basically telling that c is free and not require to move
                                                                       // what you need to do is just rearranging the string ab useing (|ab| - 1 = 1 move then use 1 more move from c)
            }
        }
    }
    cout << n - dp[n][n] << endl;
