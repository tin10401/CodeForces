...
mint i6 = mint(6).inv();
template<typename T>
T sum_of_square(ll n) { return (T)n * (n + 1) * (2 * n + 1) * i6; } // sum of 1 + 2 * 2 + 3 * 3 + 4 * 4 + ... + n * n
ll isqrt(ll n) { ll t = sqrtl(n); while(t * t < n) t++; while(t * t > n) t--; return t;}
...
void solve() {
    ...
    for(ll base = 2; base * base * base <= n; base++) {
        ll curr = base * base * base;
        int c = 3;
        while(curr <= n) {
            dp[c++] += (mint)curr * (n / curr);
            if(curr > n / base) break;
            curr *= base;
        }
    }
    {
        ...
        while(curr * curr <= n) {
            ll add = n / (curr * curr);
            ll last = isqrt(n / add);
            ans += (sum_of_square<mint>(last) - sum_of_square<mint>(curr - 1)) * add;
            curr = last + 1;
        }
    }
    mint res = n;
