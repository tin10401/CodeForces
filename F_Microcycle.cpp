...
class DSU { 
    ...
    DSU(int n) {    
        this->n = n;    
        root.rsz(n, -1), rank.rsz(n, 1), weight.rsz(n, INF);
    }
    ...
    void queries() {    
        ...
        for(int i = 1; i < n; i++) {    
            ans[find(i)].pb(i);
            if(find(i) == i) {  
                if(curr == INF || weight[find(i)] < weight[find(curr)]) curr = find(i);
            }
        }
        cout << weight[curr] << " " << ans[curr].size() << endl;    
        for(auto& it : ans[curr]) { 
            cout << it << " ";
        }
        cout << endl;
    }
...
void solve() {  
    ...
    vi ans, path;
    auto dfs = [&](auto& dfs, int node, int par, int f) -> bool {   
        path.pb(node);
        if(node == f) { 
            ans = path;
            return true;
        ...
        for(auto& nei : graph[node]) {  
            if(nei != par) {   
                if(dfs(dfs, nei, node, f)) return true;
            }
        }
        path.pop_back();
        return false;
    };
    dfs(dfs, fin.ff, -1, fin.ss);   
