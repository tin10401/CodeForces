...
void solve()
{
    ...
    {   
        cin >> arr[i];  
        f.update(i, arr[i]);    
        h.update(i, arr[i] * (n - i));  
    }   
    auto compute = [&](int x, int k)   
    {   
        return f.get(x - k) * k + h.get(x) - h.get(x - k) - (f.get(x) - f.get(x - k)) * (n - x - 1);
    };
    ...
    {   
        ...
        {   
            int l, r, k; cin >> l >> r >> k;    
            k++;    
            cout << compute(r, k) - compute(l - 1, k) << endl;  
        }   
        else    
        {   
            int x, val; cin >> x >> val;    
            f.update(x, val - arr[x]);
            h.update(x, (val - arr[x]) * (n - x));   
            arr[x] = val;   
        }   
    }
