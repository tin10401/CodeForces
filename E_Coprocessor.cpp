...
void solve() {
    ...
    queue<int> q[2];
    for(int i = 0; i < n; i++) {
        if(degree[i] == 0) q[a[i]].push(i);
    }
    auto process = [&](queue<int>& Q) -> bool {
        bool ok = false;
        while(!Q.empty()) {
            ok = true;
            auto node = Q.front(); Q.pop();
            for(auto& nei : graph[node]) {
                if(--degree[nei] == 0) q[a[nei]].push(nei);
            }
        }
        return ok;
    };
    int res = 0; 
    while(!q[0].empty() || !q[1].empty()) { // process the one without dependencies first
        process(q[0]);
        res += process(q[1]);
    }
    cout << res << endl;
